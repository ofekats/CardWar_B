...
int Game::numGamesPlayed =0;

//func can throw an exception
Game::Game(Player& p1_ref, Player& p2_ref): //constructor
p1_(p1_ref), p2_(p2_ref)
{
    if(p1_ref.getIsPlaying() == false && p2_ref.getIsPlaying() == false && p1_ref.getID() != p2_ref.getID())
    {
        numGamesPlayed++;
        this->num_of_drow_ =0;
        this->num_of_rounds_=0;
        this->filename = "gameLogs_"+to_string(numGamesPlayed)+".txt";
        ...
        {
            throw invalid_argument("Failed to open gameLogs file"); // throw an exception
        }
        ...
        Card deck_of_card[NUMBER_OF_CARD];
...
Game::~Game() //close the gameLogsFile
{
...
//put endOfGame() in every exception
//write more //
//need to see what with the test
//rewue the exception and change them to what it needs to be